...
int mickey__run_left(act_t *act, gobject_t *gobject, gobject_list_t *gobjlist)
{
	(void) gobjlist;
	if (!act->started) {
		my_printf("START running left!\n");
		if (act->sound)
...
act_t *mickey__create_run_left_act(anim_index_t *anim,
					move_index_t *move,
					...
					act_list_index_t *interdictions)
{
	...
	act = act__create("run_left", mickey__run_left, anim, move);
	if (!act)
		return NULL;
