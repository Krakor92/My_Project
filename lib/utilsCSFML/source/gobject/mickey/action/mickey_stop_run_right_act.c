...
static void mickey__start_stop_run_right(act_t *act, gobject_t *gobject,
						gobject_list_t *gobjlist)
{
	...
	(void) gobjlist;
	run = gobject__get_act(gobject, "run_right");
	if (!run)
		return;
...
//		printf("\n\n\n\n\n\n\n\n%ld\n\n\n\n\n\n\n\n", nb);
	...
	my_printf("START stop run right!\n");
	act__reset(run);
	if (act->sound)
		sfMusic_play(act->sound);
}

int mickey__stop_run_right(act_t *act, gobject_t *gobject,
				gobject_list_t *gobjlist)
{
	if (!act->started)
		mickey__start_stop_run_right(act, gobject, gobjlist);
	my_printf("%s => %s\n", gobject->name, act->name);
	gobject__do_anim(gobject, act->anim);
	gobject__do_move(gobject, act->move);
	if (act->move->finished) {
		my_printf("END stop run right!\n");
		act__reset(act);
		return 0;
	}
...
act_t *mickey__create_stop_run_right_act(anim_index_t *anim,
						move_index_t *move,
						...
						act_list_index_t *interdictions)
{
	...
	act = act__create("stop_run_right", mickey__stop_run_right,
			anim, move);
	if (!act)
		return NULL;
